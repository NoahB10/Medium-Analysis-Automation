...
class SteadyState:
    ...
    def test(self):
        if (
            abs(self.std_dev) < 0.006
        ):  # Don't go much lower than this as it will not work
            return 1
        else:
            return None
...
class DataProcessor:
    def __init__(self):
        ...
        self.output = pd.DataFrame(
            columns=["Glutamate", "Glutamine", "Glucose", "Lactate"]
        )
        self.buffer = 0
        ...
        self.thresh = [
            ...
            [0.2, 100],
            [0.2, 100],
        ]  # list for glutamate, glutamine, glucose, lactate
        self.sign = [None] * 4
    ...
    def analyze_buffer(self, index):
        for column in self.results.columns:
            ...
            steady_state = SteadyState(
                self.results.loc[index - self.buffer + 1 : index, column]
            )
            i = self.results.columns.get_loc(column)
            ss_index = steady_state.test()
            if ss_index:
                value = int(abs(self.results.loc[index, column]) * 1000) / 1000
                if not self.outputs[i] or (
                    not (
                        self.outputs[i][-1] * self.thresh[i][0]
                        <= value
                        <= self.outputs[i][-1] * self.thresh[i][1]
                    )
    ...
    def check_change(self, index):
        ...
        if self.sign[0] is not None and all(x == self.sign[0] for x in self.sign[:]):
            ...
            if not new_row.dropna(axis=1).empty and (
                self.output.empty or not (new_row.iloc[0] == self.output.iloc[-1]).all()
            ):
...
if __name__ == "__main__":
    ...
    live = (
        True  # Flag to decide if read from the file (False) or write to it live (True)
    )
    COM_PORT = "COM20"
    BAUD = 9600
    TIMEOUT = 0.5
