...
results = pd.DataFrame({'Glutamate': glutamate*gain.loc[0,'Glutamate'],'Glutamine': glutamine*gain.loc[0,'Glutamine'], 'Glucose': glucose*gain.loc[0,'Glucose'],'Lactate': lactate*gain.loc[0,'Lactate']})
...
for index, row in results.iterrows(): #modify this when I get the live feed, right now this is for saved results (note index is the row number and row is the row content)
    data.loc[index] = row
    buffer = buffer+1
    if buffer > 15: #Need to optomize the paramter
        for column in data.columns:
            ...
            if ss_index:
                ...
                if row_count and output.loc[row_count-1,column]*0.55 <= value <= output.loc[row_count-1,column]*1.35:
                    continue
                
                ss[i] = value
                ss_loc[i] = index
                if all(x is not None for x in ss):
                    output.loc[row_count] = ss + ss_loc
