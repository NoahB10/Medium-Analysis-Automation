...
def getOpenPorts():
    if sys.platform.startswith('win'):
        ports = ['COM%s' % (i + 1) for i in range(30)] #Can modify the range up to 256 depending on computer
...
def COM_Test(ports):
        """
        Check which port is the one connected to the pump by seeing which doesnt time out
        ...
        port : array of strings
            The singular COM port being checked

        """
        def Check(port):
            try:
                with serial.Serial(port,write_timeout = 1, timeout=1) as s:
                    arg = bytes(str('pump status'), 'utf8') + b'\r'
                    s.write(arg)
                    #Sometimes dont need the code below, it will connect to the correct one. other times it wont and need to varify
                    response_list = []
                    while True:
