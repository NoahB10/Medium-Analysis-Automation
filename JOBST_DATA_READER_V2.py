...
for index, row in results.iterrows(): #modify this when I get the live feed, right now this is for saved results (note index is the row number and row is the row content)
    ...
    for column in data.columns:
        ...
        #At the very beginning detect when we spike to get started 
        if index > 22:
            baseline[i]= data.loc[:index,column].mean()
            peaks, properties = find_peaks(data[column], width=22, prominence=0.8) #distance=22
            sustained_peaks = [peak for peak, width in zip(peaks, properties['widths']) if width >= 22]
            if sustained_peaks and not ss[i] :
                ss_loc[i] = index #offset to keep track of actual value
                ss[i] = data.loc[index,column]
